...
namespace QuestForCamembert
{
    ...
    public static class Program
    {
        ...
        private static CharacterProfile CreateCharacter()
        {
            ...
            Console.WriteLine("1. Danger Mouse (Strong)");
            Console.WriteLine("2. Sherlock Mouse (Intelligent)");
            Console.WriteLine("3. Micky Mouse (Charismatic)");
            Console.Write("> ");

            Character selectedCharacter = GetCharacterClass(name);
            ...
            return new CharacterProfile(selectedCharacter.Name, inventory, selectedCharacter.CharacterStats);
        }

        private static Character GetCharacterClass(string name)
        {
            var choice = ReadMenuChoice(3);

            return choice switch
            {
                1 => new DangerMouse(name),
                2 => new SherlockMouse(name),
                3 => new MickyMouse(name),
                _ => new DangerMouse(name)
            };
        }
        ...
        private static void PlayScene(Scene scene, CharacterProfile character)
        {
            ...
            if (menuChoice <= usableItems.Count)
            {
                ...
                outcome = path.ItemOutcomes[selectedItem.Key];
            }
            else
            {
